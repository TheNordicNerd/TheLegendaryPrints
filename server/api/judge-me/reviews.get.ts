export default defineEventHandler(async (event) => {
  ...
  const { productHandle, productId, page = 1, perPage = 10 } = query;
  ...
  try {
    ...
    // Build Judge.me API URL
    // Note: Reading reviews requires private API key with read_reviews scope
    ...
    if (productHandle) {
      params.append("handle", productHandle.toString());
    } else if (productId) {
    ...
    console.log("Fetching reviews from Judge.me:", {
      ...
      productHandle,
      productId,
      url: judgeMeUrl.replace(config.public.judgeMeToken, "***"),
    });
    ...
    console.log("Judge.me API response:", {
